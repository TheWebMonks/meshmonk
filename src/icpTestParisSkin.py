...
distTargetToFloat, idxTargetToFloat = kdTreeFloating.query(targetFeatures, k, eps, p, maxDist)
...
affinityFloatingToTarget = affinityFloatingToTarget + affinityTargetToFloating
rowSums = affinityFloatingToTarget.sum(axis=1, keepdims=True)
affinityFloatingToTarget = affinityFloatingToTarget / rowSums
...
"""
IMPORTANT NOTE: For the ease of implementing the ICP algorithms, we transpose our our
matrices here so the each column corresponds to a single vertex and the rows
to a different co-ordinate or feature.
"""
...
floatingFeatures[3:6,:] = floatingNormals
################################################################################
##########
#EXPORT DATA
##########
...
that to make the implementation of ICP easier, but now we have to undo that.
"""
floatingPositions = floatingPositions.transpose()
targetPositions = targetPositions.transpose()
correspondingPositions = correspondingPositions.transpose()
