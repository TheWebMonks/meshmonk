...
enum class RegistrationError {
  DegenerateInput,     // empty inputs, dim/shape mismatch, zero-range bbox,
                       // all-zero flags
  DecompositionFailed, // RigidTransformer eigendecomposition fails (Horn method
                       // / SelfAdjointEigenSolver)
  NonConvergence,      // v0.2+ only — NOT raised in v0.1
  InsufficientInliers, // fewer than 4 non-zero inlier weights after
                       // InlierDetector
};
...
struct NonrigidResult {
  ...
  VecDynFloat
      final_inlier_weights; // (N,) — computed via a final correspondence+inlier
                            // pass after registration
  Vec3Mat displacement_field; // (N,3) per-vertex displacement from original
                              // floating positions
  int iterations_run = 0;
};
...
struct PyramidResult {
  ...
  VecDynFloat
      final_inlier_weights; // (N,) — computed via a final pass at finest layer
