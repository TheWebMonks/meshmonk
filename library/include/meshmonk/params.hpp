...
// All param structs use MATLAB demo values as defaults (NOT legacy C++
// defaults). Field order is STABLE across minor versions (v0.x -> v0.x+1);
// reordering requires an ADR update. C++ callers use C++20 designated
// initializers; nanobind shim uses named nb::arg() bindings.
...
struct ViscoElasticParams {
  float sigma = 3.0f;
  int num_viscous_iterations_start =
      200; // fallback for nonrigid; pyramid auto-sets to num_iterations at
           // Python level
