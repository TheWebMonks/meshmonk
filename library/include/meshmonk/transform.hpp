...
// RigidTransform wraps a 4x4 homogeneous matrix representing a rigid (or
// similarity) transform. Semantic contract: apply(original_features) ==
// aligned_features (up to float rounding)
struct RigidTransform {
  Eigen::Matrix4f matrix = Eigen::Matrix4f::Identity();

  // Returns (*this) * rhs — matrix multiplication order (apply rhs first, then
  // *this)
  ...
  //   normals   (cols 3-5): R*n        (rotation only, no scale, no
  //   translation)
  FeatureMat apply(const FeatureMat &features) const {
    ...
    result.leftCols(3) =
        (features.leftCols(3) * R.transpose()).rowwise() + t.transpose();
