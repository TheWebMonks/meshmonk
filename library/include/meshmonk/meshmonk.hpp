...
compute_correspondences(Eigen::Ref<const FeatureMat> floating_features,
                        ...
                        Eigen::Ref<const VecDynFloat> floating_flags,
                        Eigen::Ref<const VecDynFloat> target_flags,
                        bool symmetric = true, int num_neighbours = 3,
...
compute_inlier_weights(Eigen::Ref<const FeatureMat> floating_features,
                       ...
                       float kappa = 12.0f, bool use_orientation = true);

// Compute rigid transform from features + correspondences + weights.
// IMPORTANT: returns RigidTransform (does NOT mutate floating_features in
// place). Implementation requires set_output() on RigidTransformer (REQUIRED or
// null pointer crash).
RigidTransform
...
FeatureMat
compute_nonrigid_transform(Eigen::Ref<const FeatureMat> floating_features,
                           Eigen::Ref<const FeatureMat> corresponding_features,
                           Eigen::Ref<const FacesMat> floating_faces,
                           Eigen::Ref<const VecDynFloat> floating_flags,
                           Eigen::Ref<const VecDynFloat> weights,
                           int num_smoothing_neighbours = 10,
                           float sigma = 3.0f, int num_viscous_iterations = 50,
                           int num_elastic_iterations = 50);

// Downsample a mesh.
// Returns: {features, faces, flags, original_indices}
std::tuple<FeatureMat, FacesMat, VecDynFloat, Eigen::VectorXi> downsample_mesh(
    Eigen::Ref<const FeatureMat> features, Eigen::Ref<const FacesMat> faces,
    Eigen::Ref<const VecDynFloat> flags, float downsample_ratio = 0.8f);
