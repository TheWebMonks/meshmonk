...
void Downsampler::set_output(FeatureMat &outFeatures, FacesMat &outFaces,
...
void Downsampler::update() {
  ...
  TriMesh mesh;
  convert_matrices_to_mesh(*_inFeatures, *_inFaces, *_inFlags, mesh);
  ...
  if (!addSucces) {
    meshmonk::log(meshmonk::LogLevel::Warning,
                  "registering quadric module to decimater failed!");
  }
  ...
  if (!rc) {
    ...
    meshmonk::log(meshmonk::LogLevel::Warning,
                  "  maybe no priority module or more than one were defined!");
    return;
  }
  ...
  //    std::cout << "DEBUG - downsampleratio : " << _downsampleRatio << " ||
  //    numdecimations : " << numDecimations << std::endl;
  if (numDecimations > 0) {
  ...
  if (_outOriginalIndices != NULL) {
    *_outOriginalIndices = VecDynInt::Zero(numVertices); // resizing
    bool propertyExist =
        mesh.get_property_handle(originalIndices, "originalIndices");
    if (!propertyExist) {
      throw std::runtime_error(
          "Downsampler: mesh decimation failed - could not find "
          "'originalIndices' property handle after downsampling");
    }
