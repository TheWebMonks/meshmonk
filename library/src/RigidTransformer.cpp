...
void RigidTransformer::set_input(
...
void RigidTransformer::set_parameters(bool scaling) { _scaling = scaling; }
...
void RigidTransformer::_update_transformation() {
  ...
  if ((_numElements > _numFeatures) &&
      (_numFeatures == NUM_FEATURES)) { // this should normally be the case
    ...
    meshmonk::log(meshmonk::LogLevel::Warning,
                  "input of rigid transformation expects rows to correspond "
                  "with elements, not features, and to have more elements than "
  ...
  for (size_t i = 0; i < _numElements; i++) {
    floatingCentroid +=
        (*_inWeights)[i] * floatingPositions.col(i).segment(0, 3);
    correspondingCentroid +=
        (*_inWeights)[i] * correspondingPositions.col(i).segment(0, 3);
  ...
  for (size_t i = 0; i < _numElements; i++) {
    crossVarianceMatrix += (*_inWeights)[i] * floatingPositions.col(i) *
                           correspondingPositions.col(i).transpose();
  }
  crossVarianceMatrix = crossVarianceMatrix / sumWeights -
                        floatingCentroid * correspondingCentroid.transpose();

  // ## 3. Compute the Anti-Symmetric matrix
  Mat3Float antiSymmetricMatrix =
      crossVarianceMatrix - crossVarianceMatrix.transpose();

  // ## 4. Use the cyclic elements of the Anti-Symmetric matrix to construct
  // delta
  ...
  // ### diagonal elements
  rotMatTemp(0, 0) = rotQuat[0] * rotQuat[0] + rotQuat[1] * rotQuat[1] -
                     rotQuat[2] * rotQuat[2] - rotQuat[3] * rotQuat[3];
  rotMatTemp(1, 1) = rotQuat[0] * rotQuat[0] + rotQuat[2] * rotQuat[2] -
                     rotQuat[1] * rotQuat[1] - rotQuat[3] * rotQuat[3];
  rotMatTemp(2, 2) = rotQuat[0] * rotQuat[0] + rotQuat[3] * rotQuat[3] -
                     rotQuat[1] * rotQuat[1] - rotQuat[2] * rotQuat[2];
  // ### remaining elements
  ...
  if (_scaling == true) {
    ...
    for (size_t i = 0; i < _numElements; i++) {
      ...
      Vec3Float newFloatingPos =
          rotMatTemp * (floatingPositions.block<3, 1>(0, i) -
                        floatingCentroid.segment(0, 3));
      // ### Center the corresponding position
      Vec3Float newCorrespondingPos = correspondingPositions.block<3, 1>(0, i) -
                                      correspondingCentroid.segment(0, 3);

      // ### Increment numerator and denominator
      numerator +=
          (*_inWeights)[i] * newCorrespondingPos.transpose() * newFloatingPos;
      denominator +=
          (*_inWeights)[i] * newFloatingPos.transpose() * newFloatingPos;
    }
    scaleFactor = numerator / denominator;
  }

  // ## 9. Compute the remaining translation necessary between the centroids
  Vec3Float translation =
      correspondingCentroid - scaleFactor * rotMatTemp * floatingCentroid;

  // ## 10. Compute the entire transformation matrix.
  // ### Initialize Matrices
  Mat4Float translationMatrix = Mat4Float::Identity();
  Mat4Float rotationMatrix =
      Mat4Float::Identity(); // to apply just the rotation to the surface
                             // normals later
...
void RigidTransformer::update() { _update_transformation(); } // end update
