...
void PyramidNonrigidRegistration::set_parameters(
    size_t numIterations /*= 60*/, size_t numPyramidLayers /* = 3*/,
    float downsampleFloatStart /* = 90.0f*/,
    float downsampleTargetStart /* = 90.0f*/,
    float downsampleFloatEnd /* = 0.0f*/, float downsampleTargetEnd /* = 0.0f*/,
    ...
    float inlierKappa /* = 4.0f*/, bool inlierUseOrientation /* = true*/,
  ...
  _iterationsPerLayer =
      int(std::round(float(_numIterations) / float(_numPyramidLayers)));
  if (_transformNumViscousIterationsStart > 0) {
    _viscousAnnealingRate =
        exp(log((float(_transformNumViscousIterationsEnd) + 0.5f) /
                float(_transformNumViscousIterationsStart)) /
            _numIterations); // the +0.5f is for numerical stability
  } else {
    _viscousAnnealingRate = 1.0f;
  }
  if (_transformNumElasticIterationsStart > 0) {
    _elasticAnnealingRate =
        exp(log((float(_transformNumElasticIterationsEnd) + 0.5f) /
                float(_transformNumElasticIterationsStart)) /
            _numIterations); // the +0.5f is for numerical stability
  } else {
  ...
  for (size_t i = 0; i < _numPyramidLayers; i++) {
    _viscousIterationsIntervals[i] =
        std::round(_transformNumViscousIterationsStart *
                   pow(_viscousAnnealingRate, i * _iterationsPerLayer));
    _elasticIterationsIntervals[i] =
        std::round(_transformNumElasticIterationsStart *
                   pow(_elasticAnnealingRate, i * _iterationsPerLayer));
  }
  _viscousIterationsIntervals[_numPyramidLayers] =
      _transformNumViscousIterationsEnd;
  _elasticIterationsIntervals[_numPyramidLayers] =
      _transformNumElasticIterationsEnd;
} // end set_parameters()
...
void PyramidNonrigidRegistration::update() {
  ...
  /*
  We need to do this before the pyramid iterations, because those have to be
  passed from the previous iteration to the next one. The 'ScaleShifter' class
  makes sure that the properties of the floating mesh of the previous pyramid
  scale are transferred to the current pyramid scale.
  */
  ...
  for (size_t i = 0; i < _numPyramidLayers; i++) {
    ...
    if (_numPyramidLayers > 1) {
      downsampleRatio = float(std::round(
          _downsampleFloatStart -
          i * std::round((_downsampleFloatStart - _downsampleFloatEnd) /
                         (_numPyramidLayers - 1.0))));
    }
    downsampleRatio /= 100.0f;
    // ## Set up Downsampler
    Downsampler downsampler;
    VecDynFloat floatingFlags;
    downsampler.set_input(_ioFloatingFeatures, _inFloatingFaces,
                          _inFloatingFlags);
    ...
    if (_numPyramidLayers > 1) {
      downsampleRatio = float(std::round(
          _downsampleTargetStart -
          i * std::round((_downsampleTargetStart - _downsampleTargetEnd) /
                         (_numPyramidLayers - 1.0))));
    }
    downsampleRatio /= 100.0f;
    // ## Set up Downsampler
    ...
    // # Transfer floating mesh properties from previous pyramid scale to the
    // current one.
    if (i > 0) {
      // ## Scale up
      ScaleShifter scaleShifter;
      scaleShifter.set_input(oldFloatingFeatures, oldFloatingOriginalIndices,
                             floatingOriginalIndices);
    ...
    nonrigidRegistration.set_input(&floatingFeatures, &targetFeatures,
                                   &floatingFaces, &floatingFlags,
                                   &targetFlags);
    nonrigidRegistration.set_parameters(
        _correspondencesSymmetric, _correspondencesNumNeighbours,
        _correspondencesFlagThreshold, _correspondencesEqualizePushPull,
        _inlierKappa, _inlierUseOrientation, _iterationsPerLayer,
  ...
  for (size_t j = 0; j < numFloatingFeatures; j++) {
    originalIndices(j) = j;
  }
  ScaleShifter scaleShifter;
  scaleShifter.set_input(floatingFeatures, floatingOriginalIndices,
                         originalIndices);
