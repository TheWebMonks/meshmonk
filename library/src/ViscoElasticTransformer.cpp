...
void ViscoElasticTransformer::set_input(
    const FeatureMat *const inCorrespondingFeatures,
    const VecDynFloat *const inWeights, const VecDynFloat *const inFlags,
  ...
  _flagsOutdated = true; // if the user sets new flags, we need to update our
                         // smoothing weights.
} // end set_input()
...
void ViscoElasticTransformer::set_output(FeatureMat *const ioFloatingFeatures) {
  ...
  _neighboursOutdated =
      true; // if the user sets new floating Features, we need to update our
            // neighbours, and hence our smoothing weights.
  _flagsOutdated = true;
  ...
  convert_matrices_to_mesh(
      *_ioFloatingFeatures, *_inFloatingFaces,
      _floatingMesh); // NOTE: We should do actually really be doing this EVERY
                      // TIME the user provides a different floating mesh to
                      // this class.
...
void ViscoElasticTransformer::set_parameters(size_t numNeighbours, float sigma,
  ...
  if (_numNeighbours != numNeighbours) {
    _neighboursOutdated = true; // if number of requested neighbours changes, we
                                // need to update the neighbours and weights
    _flagsOutdated = true;
  }
...
void ViscoElasticTransformer::_update_smoothing_weights() {
  ...
  Therefor, we initialize the smoothing weights as the squared distances to each
  neighbour. Next, we convert that distance to the gaussian weight. Then, we
  multiply that weight with the user inputted flags. And lastly, we normalize
  each row so that the sum of weights for each node's neighbours equals 1.0.
  */

  // # Initialize the smoothing weights as the squared distances to the
  // neighbouring nodes.
  ...
  for (size_t i = 0; i < _numElements; i++) {
    ...
    for (size_t j = 0; j < _numNeighbours; j++) {
      ...
      const float distanceSquared = _smoothingWeights(
          i, j); // smoothing weight still equals the squared distance here
      // ## Compute the gaussian weight
      const float gaussianWeight =
          std::exp(-0.5f * distanceSquared / std::pow(_sigma, 2.0f));
      ...
      // rescale the combined weight between [eps,1.0] instead of [0.0,1.0]. If
      // we wouldn't do this, all the nodes with inlierWeight equal to 0.0 would
      // end up with a deformation vector of length 0.0.
    ...
    if (sumWeight > 0.000001f) {
      ...
      meshmonk::log(meshmonk::LogLevel::Warning,
                    "Sum of smoothing weights in ViscoElastic Transformer "
                    "should never be smaller than epsilon.");
      printedWarning = true;
    }
  }
...
void ViscoElasticTransformer::_update_viscously() {
  ...
  The force field is the difference between the current floating and
  corresponding Features. A purely viscous transformation can be obtained by
  adding a regularized force fold to the total displacement field. If, however,
  that total displacement field is regularized as well, a more elastic behaviour
  is achieved.
  ...
  */
  ...
  // # 1) Determine the force field (difference between current floating and
  // corresponding # Features).
  Vec3Mat forceField =
      _inCorrespondingFeatures->leftCols(3) - _ioFloatingFeatures->leftCols(3);
  // ## Each differential vector is multiplied with the corresponding inlier
  // weight.
  // ## That ensures that patches of outliers don't move unless pulled along by
  // surrounding inliers.
  forceField = forceField.array().colwise() *
               (*_inWeights).array(); // this multiplies each row by the
                                      // corresponding inlier weight
  ...
  */
  ...
  for (size_t it = 0; it < _viscousIterations; it++) {
    for (size_t i = 0; i < _numElements; i++) {
      // ## For the current displacement, compute the weighted average of the
      // neighbouring
      // ## vectors.
      ...
      for (size_t j = 0; j < _numNeighbours; j++) {
        ...
        // rescale the weight between [eps,1.0] instead of [0.0,1.0]. If we
        // wouldn't do this, all the nodes with inlierWeight equal to 0.0 would
        // end up with a deformation vector of length 0.0.
  ...
  _oldDisplacementField = _displacementField; // save the previous displcament
                                              // field before overwriting it.
...
void ViscoElasticTransformer::_update_outlier_transformation() {
  ...
  for (size_t it = 0; it < _outlierDiffusionIterations; it++) {
    ...
    // ## Loop over the displacement vectors of the outliers (with inlier weight
    // < 0.8).
    for (size_t i = 0; i < _numElements; i++) {
      ...
      if (inlierWeight > 0.8) {
        ...
        for (size_t j = 0; j < _numNeighbours; j++) {
          ...
          // get neighbour weight and vector
          float weight =
              _smoothingWeights(i, j); // inlier weight is already incorporated
                                       // in the smoothing weight.
          neighbourVector = temporaryDisplacementField.row(neighbourIndex);
          sumWeights += weight;

          // increment the weighted average with current weighted neighbour
          // vector
          vectorAverage += weight * neighbourVector;
        }

        // ## We want diffusion of the displacement field in outlier areas, but
        // in inlier areas we don't want the
        // ## displacement to change. We therefor use the inlier weight to weigh
        // the assignment of the newly computed
        // ## averaged displacement versus its old value.
        vectorAverage /= sumWeights; // smoothing weights are already
                                     // normalized, so this should be redundant.
...
void ViscoElasticTransformer::_apply_transformation() {
  ...
  for (size_t i = 0; i < _numElements; i++) {
    _ioFloatingFeatures->row(i).head(3) +=
        (_displacementField.row(i) - _oldDisplacementField.row(i));
  }
