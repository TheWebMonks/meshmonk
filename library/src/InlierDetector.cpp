...
void InlierDetector::_smooth_inlier_weights() {
  ...
  for (size_t it = 0; it < _numSmoothingPasses; it++) {
    ...
    // ## Loop over the nodes
    for (size_t i = 0; i < _numElements; i++) {
      // ## For the current node, compute the weighted average of the
      // neighbouring inlier weights
      ...
      for (size_t j = 0; j < _numNeighbours; j++) {
        ...
        const float smoothingWeight =
            _smoothingWeights(i, j); // inlier weight is already incorporated in
      ...
      // ## Determine the weighted average inlier weight by dividing by the sum
      // of smoothing weights.
      inlierWeightAvg /=
          sumSmoothingWeights; // smoothing weights are already normalized, so
                               // this should be redundant.
      (*_ioProbability)[i] = inlierWeightAvg;
    }

    // # Multiply the resulting inlier weights with the deterministic
...
void InlierDetector::update() {
  ...
  //    _parameterList.insert(std::make_pair("beschrijving", "Wat een
  //    moeilijkheden")); _parameterList.insert(std::make_pair("inlierKappa",
  //    int(3))); std::cout << "Parameter list 'beschrijving':" <<
  //    _parameterList["beschrijving"] << std::endl;
  //
  ...
  *_ioProbability = *_inCorrespondingFlags;
  ...
  for (size_t it = 0; it < numDistanceBasedIterations; it++) {
    ...
    for (size_t i = 0; i < _numElements; i++) {
      ...
      FeatureVec difVector =
          _inCorrespondingFeatures->row(i) - _inFeatures->row(i);
    ...
    if (sigmaa < _minimalSigma) {
      sigmaa = _minimalSigma;
    } else if (sigmaa > _maximalSigma) {
      sigmaa = _maximalSigma;
    }

    lambdaa = 1.0 / (std::sqrt(2.0 * 3.14159) * sigmaa) *
              std::exp(-0.5 * _kappa * _kappa);
    ...
    for (size_t i = 0; i < _numElements; i++) {
      ...
      // ### Compute probability
      float probability =
          1.0 / (std::sqrt(2.0 * 3.14159) * sigmaa) *
          std::exp(-0.5 * distanceSquared / std::pow(sigmaa, 2.0));
  ...
  if (_useOrientation) {
    float averageOrientationInlierWeight =
        0.0f; // simply to warn the user when this is too low, they probably
              // have the normals flipped.
    for (size_t i = 0; i < _numElements; i++) {
      const Vec3Float normal = _inFeatures->row(i).tail(3);
      const Vec3Float correspondingNormal =
          _inCorrespondingFeatures->row(i).tail(3);
    ...
    if (averageOrientationInlierWeight < 0.5f) {
      meshmonk::log(
          ...
          "very low inlier weights due to surface normals. Are you sure one of "
          "the surfaces doesn't have its vertex normals flipped?");
    }
  }
