...
public:
  void set_input(FeatureMat &ioFloatingFeatures,
                 ...
                 const FacesMat &inFloatingFaces, const FacesMat &inTargetFaces,
                 const VecDynFloat &inFloatingFlags,
                 const VecDynFloat &inTargetFlags);

  void set_parameters(
      size_t numIterations = 60, size_t numPyramidLayers = 3,
      float downsampleFloatStart = 90.0f, float downsampleTargetStart = 90.0f,
      ...
      bool correspondencesEqualizePushPull = false, float inlierKappa = 4.0f,
      bool inlierUseOrientation = true, float transformSigma = 3.0f,
...
private:
  ...
  // ## Transformation
  float _viscousAnnealingRate =
      exp(log(float(_transformNumViscousIterationsEnd) /
              float(_transformNumViscousIterationsStart)) /
          _numIterations);
  float _elasticAnnealingRate =
      exp(log(float(_transformNumElasticIterationsEnd) /
