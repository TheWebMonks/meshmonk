...
void ScaleShifter::_find_matching_and_new_indices() {
  ...
  std::stable_sort(lowIndexPairs.begin(), lowIndexPairs.end(),
                   [](auto &left, auto &right) {
                     return left.first < right.first;
                   }); // This construction makes sure we only sort using the
                       // first element of the std::pair
  std::stable_sort(
      highIndexPairs.begin(), highIndexPairs.end(),
      [](auto &left, auto &right) { return left.first < right.first; });

  // # Determine matching and new nodes of the high sampled mesh.
  // ##    Determine the current index pairs between the matching nodes of the
  // high and low
  // ##    sampled mesh (using matches between the original indices). And
  // determine which nodes
  // ##    of the high sampled mesh have are new (have no matching original
  // index in the low
  // ##    sampled mesh).
  ...
  for (; counterHigh < _numHighNodes; counterHigh++) {
    ...
    if (counterLow >= _numLowNodes) {
      ...
      continue;
    } else { // check if there is a match between the original indices of the
             // high and low sampled node
      ...
      // ## if one index is smaller than the other, we have to increment that
      // index
        ...
        // ## (!) This should never occur, it means that a node was found in the
        // low sampled mesh that doesn't exist in the high sampled mesh.
        meshmonk::log(
            meshmonk::LogLevel::Warning,
            "the original indices in the low sampled mesh should be a subset "
            "of those of the high sampled mesh. Something went wrong?");
        counterLow++;
        continue;
      }
    }
  ...
  if ((_numMatchingNodes + _numNewNodes) != _numHighNodes) {
    meshmonk::log(meshmonk::LogLevel::Warning,
                  "Some nodes were missed as being new or matching nodes in "
                  "ScaleShifter.");
  }
...
void ScaleShifter::_interpolate_new_nodes() {
  ...
  */
  ...
  FeatureMat matchingNodesOldFeatures =
      FeatureMat::Zero(_numMatchingNodes, NUM_FEATURES);
  ...
  for (size_t i = 0; i < _numNewNodes; i++) {
    // ## Get the index of the new nodes of the high sampled mesh. We are
    // skipping
  ...
  for (size_t i = 0; i < _numNewNodes; i++) {
    ...
    for (size_t j = 0; j < k; j++) {
      ...
      const int matchingHighNeighbourIndex =
          _matchingIndexPairs[neighbourIndex].first;
      ...
      Vec3Float neighbourOldNormal =
          _outHighFeatures->row(matchingHighNeighbourIndex).tail(3);
      ...
      deformationField.row(newNodeIndex) +=
          weight * deformationField.row(matchingHighNeighbourIndex);
      sumWeights += weight;
    }
    // ### Normalize
  ...
  for (size_t i = 0; i < _numNewNodes; i++) {
    ...
    (_outHighFeatures->row(newNodeIndex)).head(3) +=
        deformationField.row(newNodeIndex);
  }
