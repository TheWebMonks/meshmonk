...
T unwrap_expected(tl::expected<T, meshmonk::RegistrationError> result) {
    ...
    switch (code) {
        ...
        case meshmonk::RegistrationError::InsufficientInliers:
            msg = "InsufficientInliers: fewer than 4 non-zero inlier weights after InlierDetector";
            break;
        case meshmonk::RegistrationError::DecompositionFailed:
            msg = "DecompositionFailed: RigidTransformer eigendecomposition (Horn method) failed to converge";
            break;
...
NB_MODULE(_meshmonk_core, m) {
    ...
    nb::enum_<meshmonk::RegistrationError>(m, "RegistrationError")
        ...
        .value("InsufficientInliers", meshmonk::RegistrationError::InsufficientInliers)
        .value("DecompositionFailed", meshmonk::RegistrationError::DecompositionFailed)
    ...
    nb::class_<meshmonk::CorrespondenceParams>(m, "CorrespondenceParams")
        ...
        .def_rw("equalize_push_pull", &meshmonk::CorrespondenceParams::equalize_push_pull);
    ...
    nb::class_<meshmonk::ViscoElasticParams>(m, "ViscoElasticParams")
        ...
        .def_rw("num_viscous_iterations_start", &meshmonk::ViscoElasticParams::num_viscous_iterations_start)
        .def_rw("num_viscous_iterations_end",   &meshmonk::ViscoElasticParams::num_viscous_iterations_end)
        .def_rw("num_elastic_iterations_start", &meshmonk::ViscoElasticParams::num_elastic_iterations_start)
        .def_rw("num_elastic_iterations_end",   &meshmonk::ViscoElasticParams::num_elastic_iterations_end);
    ...
    nb::class_<meshmonk::PyramidParams>(m, "PyramidParams")
        ...
        .def_rw("num_pyramid_layers", &meshmonk::PyramidParams::num_pyramid_layers);
    ...
    nb::class_<meshmonk::RigidTransform>(m, "RigidTransform")
        .def(nb::init<>())
        .def_prop_ro("matrix",
            [](const meshmonk::RigidTransform& t) {
                return t.matrix;
            })
    ...
    m.def("compute_rigid_transform",
        ...
        "corresponding_features"_a,
        "weights"_a,
        "use_scaling"_a = false,
        nb::call_guard<nb::gil_scoped_release>());

    m.def("compute_nonrigid_transform",
        [](Eigen::Ref<const meshmonk::FeatureMat>  floating_features,
           Eigen::Ref<const meshmonk::FeatureMat>  corresponding_features,
           Eigen::Ref<const meshmonk::FacesMat>    floating_faces,
           Eigen::Ref<const meshmonk::VecDynFloat> floating_flags,
           ...
           int   num_elastic_iterations)
            ...
            return meshmonk::compute_nonrigid_transform(
                ...
                floating_flags, weights,
                num_smoothing_neighbours, sigma,
        ...
        "floating_flags"_a,
        "weights"_a,
        "num_smoothing_neighbours"_a = 10,
        "sigma"_a                    = 3.0f,
        "num_viscous_iterations"_a   = 50,
        "num_elastic_iterations"_a   = 50,
        nb::call_guard<nb::gil_scoped_release>());

    m.def("downsample_mesh",
        [](Eigen::Ref<const meshmonk::FeatureMat>  features,
           Eigen::Ref<const meshmonk::FacesMat>    faces,
           Eigen::Ref<const meshmonk::VecDynFloat> flags,
           float downsample_ratio)
            -> std::tuple<meshmonk::FeatureMat, meshmonk::FacesMat,
                          meshmonk::VecDynFloat, Eigen::VectorXi> {
            return meshmonk::downsample_mesh(features, faces, flags, downsample_ratio);
        },
        "features"_a,
        "faces"_a,
        "flags"_a,
        "downsample_ratio"_a = 0.8f,
        nb::call_guard<nb::gil_scoped_release>());

    m.def("scale_shift_mesh",
        [](Eigen::Ref<const meshmonk::FeatureMat>      previous_features,
           ...
           Eigen::Ref<const Eigen::VectorXi>           new_indices)
            -> meshmonk::FeatureMat {
            return meshmonk::scale_shift_mesh(previous_features, previous_indices, new_indices);
        },
        "previous_features"_a,
        "previous_indices"_a,
        "new_indices"_a,
        nb::call_guard<nb::gil_scoped_release>());
    ...
    m.def("set_log_level",
        [](meshmonk::LogLevel level) {
            meshmonk::set_log_level(level);
        },
        "level"_a);
}
