...
def test_ci_no_macos13():
    ...
    assert "macos-13" not in ci_content, (
...
def test_ci_test_command_excludes_silent_logger():
    ...
    ci_content = (WORKSPACE / ".github/workflows/ci.yml").read_text()
    assert "test_silent_logger" in ci_content, (
...
def test_cmake_stable_abi():
    ...
    assert "STABLE_ABI" in cmake_content, (
...
def test_pyproject_version_bump():
    ...
    with open(WORKSPACE / "pyproject.toml", "rb") as f:
        data = tomllib.load(f)
    assert data["project"]["version"] == "0.3.0.dev0", (
...
def test_pyproject_has_pyyaml():
    ...
    assert any("pyyaml" in dep.lower() for dep in dev_deps), (
...
def test_pyproject_has_tomli():
    ...
    with open(WORKSPACE / "pyproject.toml", "rb") as f:
        data = tomllib.load(f)
    dev_deps = data["project"]["optional-dependencies"]["dev"]
    assert any("tomli" in dep.lower() for dep in dev_deps), (
...
def test_pyproject_has_cibuildwheel_section():
    ...
    with open(WORKSPACE / "pyproject.toml", "rb") as f:
        data = tomllib.load(f)
    assert "cibuildwheel" in data.get("tool", {}), (
...
def test_init_no_hardcoded_version():
    ...
    init_content = (WORKSPACE / "meshmonk/__init__.py").read_text()
    assert '__version__ = "0.2.0"' not in init_content, (
...
def test_init_has_fallback_version():
    ...
    init_content = (WORKSPACE / "meshmonk/__init__.py").read_text()
    assert "0.0.0.dev0" in init_content, (
...
def test_adr001_has_amendment():
    ...
    adr_content = (WORKSPACE / "docs/decisions/ADR-001-meshmonk-modernization.md").read_text()
    assert "Amendment (v0.3 implementation" in adr_content, (
...
def test_adr001_amendment_mentions_12_wheels():
    ...
    adr_content = (WORKSPACE / "docs/decisions/ADR-001-meshmonk-modernization.md").read_text()
    assert "12 wheels" in adr_content, (
...
def test_cmake_openmp_comment():
    ...
    cmake_content = (WORKSPACE / "CMakeLists.txt").read_text()
    assert "OpenMP" in cmake_content, (
