# MeshMonk Hotspot Profile — 2026-04-24 (`239b4cb`)
...
- **Repo SHA**: `239b4cbfe2b1dffe58fc0dd6561c2b698bfbbad3`
- **OMP_NUM_THREADS**: 1
- **EIGEN_DONT_PARALLELIZE**: 1
- **Run command**: `python -m profiling.run_profile --tiers 100k --modes nonrigid --runs 5 --warmup 1 --seed 42`
- **Seed**: 42
- **ScopedTimer overhead**: 34 ns/scope (median of 3 × profiling_calibrate(1_000_000); individual: 34, 35, 33 ns/scope)
...
|---|---|---|---|---|---|---|
| `NonrigidRegistration::update` | 34.7 | 68100.6360 | 1 | 1.5x | 15 |  |
| `SymmetricCorrespondenceFilter::update` | 22.2 | 217.0383 | 201 | 1.3x | 3015 |  |
| `CorrespondenceFilter::update` | 16.3 | 79.4533 | 402 | 1.2x | 6030 |  |
| `NeighbourFinder::update` | 15.2 | 74.0979 | 403 | 1.2x | 6045 |  |
| `NeighbourFinder::tree_query` | 4.3 | 0.0002 | 40300000 | 1.0x | 604500000 |  |
| `ViscoElasticTransformer::_update_viscously` | 3.2 | 31.1635 | 200 | 1.0x | 3000 |  |
| `ViscoElasticTransformer::_update_elastically` | 3.1 | 30.9439 | 200 | 1.0x | 3000 |  |
| `InlierDetector::update` | 0.5 | 5.2990 | 201 | 1.0x | 3015 |  |
| `ViscoElasticTransformer::_apply_transformation` | 0.4 | 3.6423 | 200 | 1.0x | 3000 |  |
| `ViscoElasticTransformer::_update_outlier_transformation` | 0.2 | 1.7095 | 200 | 1.0x | 3000 |  |
| `ViscoElasticTransformer::_update_smoothing_weights` | 0.0 | 2.5900 | 1 | 1.0x | 15 |  |
...
- **`NonrigidRegistration::update`** (nonrigid, 100K tier): share=34.7%, Amdahl ceiling = 1/(1−0.347) = 1.5×
...
- **`NeighbourFinder::update`** (nonrigid, 100K tier): share=15.2%, Amdahl ceiling = 1/(1−0.152) = 1.2×
  - Scaling exponent k: NaN
  - Suggested tool: OpenMP (ms/call >> fork-join cost)
