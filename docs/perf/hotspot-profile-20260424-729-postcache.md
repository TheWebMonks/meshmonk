...
- **Seed**: 42
- **ScopedTimer overhead**: 33 ns/scope (median of 3 × profiling_calibrate(1_000_000); individual: 33, 33, 33 ns/scope)
...
|---|---|---|---|---|---|---|
| `NonrigidRegistration::update` | 37.2 | 62468.5670 | 1 | 1.6x | 15 |  |
| `SymmetricCorrespondenceFilter::update` | 22.5 | 188.0854 | 201 | 1.3x | 3015 |  |
| `CorrespondenceFilter::update` | 15.4 | 64.4237 | 402 | 1.2x | 6030 |  |
| `NeighbourFinder::update` | 14.2 | 59.0912 | 403 | 1.2x | 6045 |  |
| `ViscoElasticTransformer::_update_viscously` | 3.7 | 30.9025 | 200 | 1.0x | 3000 |  |
| `ViscoElasticTransformer::_update_elastically` | 3.7 | 30.8136 | 200 | 1.0x | 3000 |  |
| `NeighbourFinder::tree_query` | 2.2 | 0.0001 | 40300000 | 1.0x | 604500000 | * UNRELIABLE |
| `InlierDetector::update` | 0.6 | 5.3187 | 201 | 1.0x | 3015 |  |
| `ViscoElasticTransformer::_apply_transformation` | 0.4 | 3.7201 | 200 | 1.0x | 3000 |  |
| `ViscoElasticTransformer::_update_outlier_transformation` | 0.2 | 1.7133 | 200 | 1.0x | 3000 |  |
| `ViscoElasticTransformer::_update_smoothing_weights` | 0.0 | 2.6200 | 1 | 1.0x | 15 |  |

*UNRELIABLE = ms/call < 5× timer overhead (33 ns/scope = 0.000033 ms). Excluded from recommendations.*
...
- **`NonrigidRegistration::update`** (nonrigid, 100K tier): share=37.2%, Amdahl ceiling = 1/(1−0.372) = 1.6×
  - Scaling exponent k: NaN
  - Suggested tool: OpenMP (ms/call >> fork-join cost)
  - Fork-join cost: 15 µs (0.0% of label wall time)

- **`SymmetricCorrespondenceFilter::update`** (nonrigid, 100K tier): share=22.5%, Amdahl ceiling = 1/(1−0.225) = 1.3×
  - Scaling exponent k: NaN
  - Suggested tool: OpenMP (ms/call >> fork-join cost)
  - Fork-join cost: 3015 µs (0.0% of label wall time)

- **`CorrespondenceFilter::update`** (nonrigid, 100K tier): share=15.4%, Amdahl ceiling = 1/(1−0.154) = 1.2×
  - Scaling exponent k: NaN
  - Suggested tool: OpenMP (ms/call >> fork-join cost)
  - Fork-join cost: 6030 µs (0.0% of label wall time)

- **`NeighbourFinder::update`** (nonrigid, 100K tier): share=14.2%, Amdahl ceiling = 1/(1−0.142) = 1.2×
  - Scaling exponent k: NaN
  - Suggested tool: OpenMP (ms/call >> fork-join cost)
...
Labels with ms/call < 5× timer overhead (33 ns/scope = 0.000033 ms).
